...
namespace ppp {
    ...
    public:
        ...
        static bool             TryParse(const ppp::string& s, DateTime& dateTime) noexcept {
            ppp::string& sx = const_cast<ppp::string&>(s);
    ...
    public:
        ppp::string             ToString(const ppp::string& s) noexcept {
            ppp::string& sx = const_cast<ppp::string&>(s);
            return ToString(sx.data(), (int)sx.length());
        }
        ppp::string             ToString(const char* format) noexcept {
            return ToString(format, -1);
        }
        ppp::string             ToString(const char* format, int len) noexcept;
