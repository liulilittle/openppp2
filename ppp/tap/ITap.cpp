...
namespace ppp
{
    namespace tap
    {
        ...
        bool ITap::Output(const std::shared_ptr<Byte>& packet, int packet_size) noexcept
        {
            ...
            std::shared_ptr<ITap> self = shared_from_this();
            _context->dispatch(
                [self, this, stream, packet, packet_size]() noexcept 
                { 
                    auto buffers = boost::asio::buffer(packet.get(), packet_size);
                    if (!stream->is_open())
                    {
                        return false;
                    }

                    auto ac = [self, this, stream, packet](const boost::system::error_code& ec, std::size_t sz) noexcept
                        {
                            if (ec == boost::system::errc::operation_canceled)
                            {
                                Finalize();
                    ...
                    boost::asio::async_write(*stream, buffers, ac);
                    return true;
                });
            return true;
        }
