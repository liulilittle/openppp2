    ...
    namespace net {
        ...
        bool Socket::SetTypeOfService(int fd, int tos) noexcept {
            ...
            if (SOCKET_RESTRICTIONS_.IP_TOS_ON) {
                any |= ::setsockopt(fd, IPPROTO_IP, IPV6_TCLASS, (char*)&b, sizeof(b)) == 0;
            }
#else
#if defined(IPV6_TCLASS)
            if (SOCKET_RESTRICTIONS_.IPV6_TCLASS_ON) {
...
#endif
            if (SOCKET_RESTRICTIONS_.IP_TOS_ON) {
                any |= ::setsockopt(fd, SOL_IP, IP_TOS, (char*)&b, sizeof(b)) == 0;
            }
#endif
            return any;
        }
        ...
        bool Socket::AcceptLoopbackAsync(
            ...
            boost::asio::post(acceptor_->get_executor(),
                [context_, acceptor_, callback, socket_]() noexcept {
                    acceptor_->async_accept(*socket_,
                        [context_, acceptor_, callback, socket_](boost::system::error_code ec) noexcept {
                            ...
                            AcceptLoopbackAsync(*acceptor_, callback);
                        });
                });
            return true;
        }
