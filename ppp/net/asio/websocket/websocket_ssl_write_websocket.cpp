    ...
    namespace net {
        namespace asio {
            bool sslwebsocket::Write(const void* buffer, int offset, int length, const AsynchronousWriteCallback& cb) noexcept {
                ...
                const std::shared_ptr<boost::asio::io_context> context = context_;
                if (NULL == context) {
                    return false;
                }

                const std::shared_ptr<sslwebsocket> self = shared_from_this();
                auto complete_do_async_write_callback = [self, this, cb, buffer, offset, length, ssl_websocket]() noexcept {
                ...
                context->dispatch(complete_do_async_write_callback);
                return true;
            }
        }
    }
