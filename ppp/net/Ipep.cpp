...
namespace ppp {
    namespace net {
        ...
        int Ipep::ToDnsAddresses(const ppp::string& s, ppp::vector<boost::asio::ip::address>& out) noexcept {
        ...
        bool Ipep::GetAddressByHostName(boost::asio::io_context& context, const ppp::string& hostname, int port, const GetAddressByHostNameCallback& callback) noexcept {
            ...
            auto servers = ppp::net::asio::vdns::servers;
            if (NULL == servers) {
                return false;
            }

            return ppp::net::asio::vdns::ResolveAsync(context, hostname.data(), PPP_RESOLVE_DNS_TIMEOUT, *servers, 
                [servers, callback, port](const boost::asio::ip::address& address) noexcept {
                    IPEndPoint ep = IPEndPoint::ToEndPoint(boost::asio::ip::udp::endpoint(address, port));
                    IPEndPoint* p = NULL;
                    if (!IPEndPoint::IsInvalid(ep)) {
                        p = (IPEndPoint*)&reinterpret_cast<const char&>(ep);
                    }
