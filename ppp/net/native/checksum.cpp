    ...
    namespace net
    {
        namespace native
        {
            ...
            struct ip_hdr* ip_hdr::Parse(const void* packet, int len) noexcept
            {
                ...
                if (ip_proto == IP_PROTO_UDP || ip_proto == IP_PROTO_TCP || ip_proto == IP_PROTO_ICMP)
                {
                    return iphdr;
                }
...
#pragma pack(pop)
                ...
                ppp::string ExtractHost(const Byte* szPacketStartPos, int nPacketLength) noexcept
                {
                    ...
                    if (auto pioContext = ppp::threading::Executors::GetCurrent(); NULL != pioContext)
                    {
                        pioBuffers = ppp::threading::Executors::GetCachedBuffer(pioContext);
                    }
