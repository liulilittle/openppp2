...
#endif
                dns_hdr {
                    uint16_t                                                                usTransID;         // 事务ID - 用于匹配查询和响应
                    uint16_t                                                                usFlags;           // 标志字段 - 包含各种控制标志
                    uint16_t                                                                usQuestionCount;   // 问题部分记录数 
                    uint16_t                                                                usAnswerCount;     // 回答部分记录数
                    uint16_t                                                                usAuthorityCount;  // 授权部分记录数
                    uint16_t                                                                usAdditionalCount; // 附加部分记录数
                };
#pragma pack(pop)
                
                static constexpr int MAX_DOMAINNAME_LEN                                     = 255; /* 最大域名长度: 253字符 + . = 254字节 或 254字符 + . = 255字节 */
                ...
                static constexpr int DNS_TYPE_A                                             = 0x0001; // A记录 - IPv4地址
                static constexpr int DNS_TYPE_AAAA                                          = 0x001c; // AAAA记录 - IPv6地址
                static constexpr int DNS_TYPE_CNAME                                         = 0x0005; // CNAME记录 - 规范名称(别名)
                static constexpr int DNS_CLASS_IN                                           = 0x0001; // IN类 - 互联网
