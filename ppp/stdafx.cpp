...
namespace ppp {
    ...
    ppp::string GuidToString(const boost::uuids::uuid& uuid) noexcept {
        ...
        return result; /* boost::uuids::to_string(uuid); */
    }
    ...
    ppp::string PrintToString(std::size_t padding_length, char padding_char, const char* format, va_list args) noexcept {
        int buffer_size = 256;
        std::shared_ptr<char> buffer = make_shared_alloc<char>(buffer_size);

        int length = 0;
        if (NULL != buffer) {
            length = vsnprintf(buffer.get(), buffer_size, format, args);
            if (length > 0) {
                if (length > buffer_size) {
                    buffer = make_shared_alloc<char>(buffer_size = length + 1);
                    length = NULL != buffer ? vsnprintf(buffer.get(), buffer_size, format, args) : 0;
                }
            }
        ...
        if (NULL == buffer || length < 1) {
            return ppp::string();
        }

        ppp::string result = ppp::string(buffer.get(), length);
