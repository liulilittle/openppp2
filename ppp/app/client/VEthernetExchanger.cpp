        ...
        namespace client {
                                ...
                                if (y && EchoLanToRemoteExchanger(transmission, y) > -1) {
                                    ExchangeToEstablishState(); {
                                        transmission_ = transmission; {
                                            ...
                                            if (StaticEchoAllocatedToRemoteExchanger(y)) {
            ...
            bool VEthernetExchanger::OnStatic(const ITransmissionPtr& transmission, int session_id, int remote_port, YieldContext& y) noexcept {                
                ...
                if (remote_port == IPEndPoint::MinPort || session_id == 0) {
                    ...
                    // Compute and generate the address endpoints of the cached VPN remote exchanger.
                    boost::asio::ip::udp::endpoint serverEP = static_echo_server_ep_;
                    if (int server_port = serverEP.port(); server_port > IPEndPoint::MinPort && server_port <= IPEndPoint::MaxPort) {
                        static_echo_exchanger_ep_ = serverEP;
                    }
                    else {
                        boost::asio::ip::address remoteIP = server_url_.remoteEP.address();
                        static_echo_exchanger_ep_ = boost::asio::ip::udp::endpoint(remoteIP, remote_port);
                    }
                }
            ...
            bool VEthernetExchanger::StaticEchoSwapAsynchronousSocket() noexcept {
                ...
                if (static_echo_timeout_ != UINT64_MAX && switcher_->StaticMode(NULL)) {
                    ...
                    if (now >= static_echo_timeout_) {
                        ...
                        return YieldContext::Spawn(*context,
            ...
            bool VEthernetExchanger::StaticEchoPacketToRemoteExchanger(const std::shared_ptr<Byte>& packet, int packet_length) noexcept {
                ...
                boost::asio::ip::udp::endpoint serverEP = static_echo_exchanger_ep_;
                                ...
                                *bytes_transferred = sz;
                                if (ec) {
                                    *bytes_transferred = -1;
                                    ...
                                    std::shared_ptr<VirtualEthernetPacket> packet = my->StaticEchoReadPacket(my->buffer_.get(), *bytes_transferred);
                                    if (NULL != packet) {
                                        if (auto statistics = my->switcher_->GetStatistics(); NULL != statistics) {
                                            statistics->AddIncomingTraffic(*bytes_transferred);
                                        }
                ...
                return YieldContext::Spawn(*context,
                    [self, this, socket_weak](YieldContext& y) noexcept -> bool {
                        for (;;) {
                            ...
                            std::shared_ptr<ppp::transmissions::ITransmissionQoS> qos = switcher_->GetQoS();
                            if (NULL != qos) {
            ...
            boost::asio::ip::udp::endpoint VEthernetExchanger::StaticEchoRemoteEndPoint(boost::asio::ip::udp::endpoint* remoteEP) noexcept {
                boost::asio::ip::udp::endpoint snow = static_echo_server_ep_;
                if (NULL != remoteEP) {
                    if (int server_port = remoteEP->port(); server_port > IPEndPoint::MinPort && server_port <= IPEndPoint::MaxPort) {
                        static_echo_exchanger_ep_ = *remoteEP;
                    }

                    static_echo_server_ep_ = *remoteEP;
                }
            ...
            bool VEthernetExchanger::StaticEchoOpenAsynchronousSocket(StaticEchoDatagarmSocket& socket, YieldContext& y) noexcept {
                ...
                boost::asio::ip::address remoteIP = static_echo_server_ep_.address();
                if (int server_port = static_echo_server_ep_.port(); server_port <= IPEndPoint::MinPort || server_port > IPEndPoint::MaxPort) {
                    remoteIP = server_url_.remoteEP.address();
                }

                if (remoteIP.is_v4()) {
                    opened = ppp::coroutines::asio::async_open<boost::asio::ip::udp::socket>(y, socket, boost::asio::ip::udp::v4());
                }
                elif(remoteIP.is_v6()) {
                    opened = ppp::coroutines::asio::async_open<boost::asio::ip::udp::socket>(y, socket, boost::asio::ip::udp::v6());
                }
                else {
                    return false;
                }

                opened = opened && !disposed_;
                if (!opened) {
                    return false;
                }

                boost::asio::ip::address bindIP = boost::asio::ip::address_v4::any();
                if (remoteIP.is_v6()) {
                    bindIP = boost::asio::ip::address_v6::any();
                }

                bool ok = false;
                for (;;) {
                    opened = Socket::OpenSocket(socket, bindIP, IPEndPoint::MinPort, opened);
                    if (!opened) {
                        break;
                    }
