...
namespace ppp {
    namespace app {
        namespace client {
            namespace http {
                static class VEthernetHttpProxyConnectionStaticVariable final {
                public:
                ...
                static bool ProtocolAsyncReadSome(ppp::io::MemoryStream& protocol_array, VEthernetHttpProxyConnection::YieldContext& y, boost::asio::ip::tcp::socket& socket) noexcept {
                    char buffers[1024];
                    int transferred_bytes = ppp::coroutines::asio::async_read_some(socket, boost::asio::buffer(buffers, sizeof(buffers)), y);
                    if (transferred_bytes < 1) {
                        return false;
                    }

                    return protocol_array.Write(buffers, 0, transferred_bytes);
                }
                ...
                bool VEthernetHttpProxyConnection::ProcessHandshaking(YieldContext& y) noexcept {
                    for (;;) {
                        ...
                        if (!ProtocolAsyncReadSome(protocol_array, y, *socket_)) {
                            return false;
                        }
                ...
                bool VEthernetHttpProxyConnection::WriteToLocal(YieldContext& y, const void* messages, int messages_size) noexcept {
                    ...
                    return transmission->Write(y, messages, messages_size);
                }
