        ...
        namespace client {
            namespace http {
                ...
                bool VEthernetHttpProxyConnection::SendBufferToPeer(YieldContext& y, const void* messages, int messages_size) noexcept {
                    ...
                    if (VirtualEthernetTcpipConnectionPtr connection = this->connection_; NULL != connection) {
                        return connection->SendBufferToPeer(y, messages, messages_size);
                    }

                    if (std::shared_ptr<RinetdConnection> connection = this->connection_rinetd_; NULL != connection) {
                        std::shared_ptr<boost::asio::ip::tcp::socket> socket = connection->GetRemoteSocket(); 
                        if (NULL == socket) {
                            return false;
                        }
                ...
                bool VEthernetHttpProxyConnection::ConnectBridgeToPeer(const std::shared_ptr<ProtocolRoot>& protocolRoot, YieldContext& y) noexcept {
                    ...
                    std::shared_ptr<ppp::transmissions::ITransmission> transmission = exchanger_->ConnectTransmission(context_, y);
                    if (NULL == transmission) {
                        return false;
                    }

                    std::shared_ptr<VEthernetTcpipConnection> connection =
                        make_shared_object<VEthernetTcpipConnection>(self, configuration, context_, exchanger_->GetId(), socket);
                    if (NULL == connection) {
