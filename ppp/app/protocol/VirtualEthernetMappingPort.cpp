    ...
    namespace app {
        namespace protocol {
            ...
            void VirtualEthernetMappingPort::Finalize() noexcept {
                ...
                if (disposed != TRUE) {
                    if (std::shared_ptr<Server> server = std::move(server_); NULL != server) {
                        server_.reset();
                    ...
                    if (std::shared_ptr<Client> client = std::move(client_); NULL != client) {
                        client_.reset();
...
#ifdef VIRTUALETHERNETMAPPINGPORT_SOCKET_OPENNETWORKSOCKET
...
#undef VIRTUALETHERNETMAPPINGPORT_SOCKET_OPENNETWORKSOCKET
            ...
            bool VirtualEthernetMappingPort::OpenFrpServer(const VirtualEthernetLoggerPtr& logger) noexcept {
                ...
                if (tcp_) {
                    ...
                    auto fx = make_shared_object<ppp::net::Socket::AcceptLoopbackCallback>(
                    ...
                    return ppp::net::Socket::AcceptLoopbackAsync(acceptor, fx);
                }
                else {
            ...
            bool VirtualEthernetMappingPort::Update(UInt64 now) noexcept {
                ...
                if (std::shared_ptr<Server> server = server_; NULL != server) {
                    ppp::collections::Dictionary::UpdateAllObjects(server->socket_connections_, now);
                }

                if (std::shared_ptr<Client> client = client_; NULL != client) {
            ...
            bool VirtualEthernetMappingPort::Server::Connection::SendToFrpUser(const void* packet, int packet_size) noexcept {
                ...
                return WriteBytes(messages, packet_size, fx);
            }

            bool VirtualEthernetMappingPort::Server::Connection::DoWriteBytes(std::shared_ptr<Byte> packet, int offset, int packet_length, const std::shared_ptr<AsynchronousWriteBytesCallback>& cb) noexcept {
                int connection_state = connection_stated_.load();
                if (connection_state != 3) {
                    return false;
                }
            ...
            VirtualEthernetMappingPort::Server::Server(VirtualEthernetMappingPort* owner) noexcept
                ...
                socket_source_buf_ = ppp::threading::Executors::GetCachedBuffer(owner->context_.get());
            }
                ...
                for (;;) {
                    ...
                    if (ok) {
                        ...
                        while (ok) {
                            ...
                            logger->MPConnect(linklayer_->GetId(), transmission->GetRemoteEndPoint(), localEP, remoteEP);
                            break;
                        }
                    }
            ...
            bool VirtualEthernetMappingPort::Client::Connection::DoWriteBytes(std::shared_ptr<Byte> packet, int offset, int packet_length, const std::shared_ptr<AsynchronousWriteBytesCallback>& cb) noexcept {
                int connection_state = connection_stated_.load();
                if (connection_state != 3) {
                    return false;
                }
            ...
            VirtualEthernetMappingPort::Client::DatagramPort::DatagramPort(const std::shared_ptr<VirtualEthernetMappingPort>& mapping_port, const std::shared_ptr<Client>& client, const boost::asio::ip::udp::endpoint& natEP) noexcept
                ...
                , transmission_(mapping_port->transmission_) {
                nat_ep_ = natEP;
                buffer_chunked_ = ppp::threading::Executors::GetCachedBuffer(mapping_port->context_.get());
                Update();
            }
                    ...
                    [self, this](boost::system::error_code ec, std::size_t sz) noexcept {
                        if (ec == boost::system::errc::success) {
                            ...
                            if (sz > 0) {
                                ITransmissionPtr transmission = mapping_port_->GetTransmission();
                                if (NULL != transmission) {
                                    ok = linklayer_->DoFrpSendTo(transmission,
                                        mapping_port_->in_,
                                        mapping_port_->remote_port_,
                                        nat_ep_,
                                        buffer_chunked_.get(),
