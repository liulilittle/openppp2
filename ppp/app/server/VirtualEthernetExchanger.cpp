    ...
    namespace app {
        namespace server {
            VirtualEthernetExchanger::VirtualEthernetExchanger(
                ...
                for (;;) {
                    if (ITransmissionPtr transmission = transmission_; NULL != transmission) {
            ...
            void VirtualEthernetExchanger::Finalize() noexcept {
                exchangeof(disposed_, true); {
                    ...
                    if (VirtualInternetControlMessageProtocolPtr echo = std::move(echo_); NULL != echo) {
                        echo_.reset();
                        echo->Dispose();
                    }

                    if (std::shared_ptr<VirtualInternetControlMessageProtocolStatic> static_echo = std::move(static_echo_); NULL != static_echo) {
                        static_echo_.reset();
                        static_echo->Dispose();
                    }

                    if (ITransmissionPtr transmission = std::move(transmission_); NULL != transmission) {
                        transmission_.reset();
            ...
            bool VirtualEthernetExchanger::ForwardNatPacketToDestination(Byte* packet, int packet_length, YieldContext& y) noexcept {
                ...
                VES::NatInformationPtr nat = switcher_->FindNatInformation(destination);
                if (NULL == nat) {
                    return false;
                }
