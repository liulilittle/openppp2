...
> <img src="https://img.icons8.com/color/24/000000/law.png" width="20" height="20"> 违规使用将承担法律责</img>
...
### <img src="https://icons.veryicon.com/png/o/business/icons-related-to-operation-and-maintenance/cloud-gateway.png" width="20" height="20"> 虚拟网关地址默认值
| 平台 | 默认值 |
|------|--------|
| Windows | `10.0.0.0` |
| Linux | `10.0.0.1` |
...
### <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRW-HldTH5IkPK9M8NePfmj0f6EeeCDEI9SVw&s" width="20" height="20"> 符号说明
| 符号 | 说明 |
|:----:|------|
| `[ ]` | 可选参数 |
| `< >` | 必填参数 |
...
| 参数名      | 类型   | 默认值 | 说明                                 |
|-------------|--------|--------|--------------------------------------|
| concurrent  | int    | 1      | [并发线程数](#concurrent-number-guide) |
| cdn         | array  | [80,443] | 使用的CDN端口列表                   |

### 🔑 加密设置 (key)

| 参数名          | 类型    | 值示例               | 说明                          |
|-----------------|---------|----------------------|-----------------------------|
| kf             | int     | 154543927           | 密钥生成因子                 |
| kx             | int     | 128                 | 插花因子                     |
| kl             | int     | 10                  | NOP最小位                    |
| kh             | int     | 12                  | NOP最大位                   |
| sb             | int     | 1000                | 动态滑板大小(字节)           |
| protocol       | string  | aes-128-cfb         | 协议层加密算法               |
| protocol-key   | string  | N6HMzdUs7IUnYHwq    | 协议层加密密钥               |
| transport      | string  | aes-256-cfb         | 传输层加密算法               |
| transport-key  | string  | HWFweXu2g5RVMEpy    | 传输层加密密钥               |
| masked         | bool    | false               | 是否启用流量混淆             |
| plaintext      | bool    | false               | 是否允许明文传输             |
| delta-encode   | bool    | false               | 是否启用增量编码             |
| shuffle-data   | bool    | false               | 是否启用数据随机化           |

### 📡 网络接口 (ip)

| 参数名      | 类型   | 值示例         | 说明                     |
|-------------|--------|----------------|------------------------|
| public      | string | 192.168.0.24   | 公网IP地址              |
| interface   | string | 192.168.0.24   | 本地监听接口IP          |

### 💾 虚拟内存 (vmem)

| 参数名 | 类型   | 值示例   | 说明                     |
|--------|--------|----------|------------------------|
| size   | int    | 4096     | 内存池大小(MB)          |
| path   | string | "./{}"   | 内存文件存储路径         |

### 🔌 TCP配置

| 参数名               | 类型   | 值示例 | 说明                          |
|----------------------|--------|--------|-----------------------------|
| inactive.timeout     | int    | 300    | 连接空闲超时时间(秒)          |
| connect.timeout      | int    | 5      | 连接建立超时时间(秒)          |
| listen.port          | int    | 20000  | 监听端口                     |
| cwnd                 | int    | 0      | 拥塞窗口大小(自动调整)        |
| rwnd                 | int    | 0      | 接收窗口大小(自动调整)        |
| turbo                | bool   | true   | 启用TCP加速                 |
| backlog              | int    | 511    | 最大挂起连接数               |
| fast-open            | bool   | true   | 启用TCP Fast Open          |
...
| cwnd                        | int    | 0                          | 拥塞窗口大小                 |
| rwnd                        | int    | 0                          | 接收窗口大小                 |
| inactive.timeout            | int    | 72                         | 连接空闲超时(秒)             |
| dns.timeout                 | int    | 4                          | DNS查询超时(秒)              |
| dns.ttl                     | int    | 60                         | DNS缓存TTL(秒)              |
| dns.cache                   | bool   | true                       | 启用DNS缓存                 |
| dns.redirect                | string | "0.0.0.0"                  | DNS重定向地址                |
| listen.port                 | int    | 20000                      | 监听端口                     |
| static.keep-alived          | array  | [1,5]                      | 保活间隔[最小值,最大值](秒)  |
| static.dns                  | bool   | true                       | 启用静态DNS服务             |
| static.quic                 | bool   | true                       | 启用QUIC支持                |
| static.icmp                 | bool   | true                       | 启用ICMP支持                |
| static.aggligator           | int    | 4                          | 宽频聚合器链接数             |
| static.servers              | array  | ["1.0.0.1:20000",...]      | 静态服务器地址列表           |

### 🔄 多路复用 (mux)

| 参数名               | 类型   | 值示例 | 说明                          |
|----------------------|--------|--------|-----------------------------|
| connect.timeout      | int    | 20     | 连接建立超时(秒)             |
| inactive.timeout     | int    | 60     | 流空闲超时(秒)               |
| congestions          | int    | 134217728 | 最大拥塞控制窗口(字节)      |
| keep-alived          | array  | [1,20] | 保活间隔[最小值,最大值](秒)  |

### 🌐 WebSocket配置

| 参数名                          | 类型   | 值示例                                   | 说明                          |
|---------------------------------|--------|------------------------------------------|-----------------------------|
| host                            | string | starrylink.net                           | 服务器域名                    |
| path                            | string | /tun                                     | WebSocket路径                |
| listen.ws                       | int    | 20080                                    | HTTP监听端口                 |
| listen.wss                      | int    | 20443                                    | HTTPS监听端口                |
| ssl.certificate-file            | string | starrylink.net.pem                       | SSL证书文件                  |
| ssl.certificate-chain-file      | string | starrylink.net.pem                       | SSL证书链文件                |
| ssl.certificate-key-file        | string | starrylink.net.key                       | SSL私钥文件                  |
| ssl.certificate-key-password    | string | test                                     | SSL私钥密码                  |
| ssl.ciphersuites                | string | TLS_AES_256_GCM_SHA384:...               | 加密套件列表                 |
| verify-peer                     | bool   | true                                     | 验证客户端证书               |
| http.error                      | string | Status Code: 404; Not Found              | HTTP错误响应内容             |
| http.request                    | object | {Cache-Control: "no-cache", ...}         | 自定义HTTP请求头             |
| http.response                   | object | {Server: "Kestrel"}                      | 自定义HTTP响应头             |

### 🖥️ 服务器配置 (server)

| 参数名       | 类型   | 值示例                               | 说明                          |
|--------------|--------|--------------------------------------|-----------------------------|
| log          | string | ./ppp.log                            | 日志文件路径                 |
| node         | int    | 1                                    | 服务器节点ID                 |
| subnet       | bool   | true                                 | 启用子网分配                 |
| mapping      | bool   | true                                 | 启用端口映射                 |
| backend      | string | ws://192.168.0.24/ppp/webhook        | 管理后台URL                  |
| backend-key  | string | HaEkTB55VcHovKtUPHmU9zn0NjFmC6tff   | 管理后台认证密钥             |

### 💻 客户端配置 (client)

| 参数名                     | 类型   | 值示例                                   | 说明                          |
|----------------------------|--------|------------------------------------------|-----------------------------|
| guid                       | string | {F4569208-BB45-4DEB-B115-0FEA1D91B85B}   | 客户端唯一标识符             |
| server                     | string | ppp://192.168.0.24:20000/                | 服务器连接地址               |
| server-proxy               | string | http://user:pass@192.168.0.18:8080/      | 连接服务器的代理地址         |
| bandwidth                  | int    | 10000                                    | 带宽限制(Kbp/s)               |
| reconnections.timeout      | int    | 5                                        | 重连等待时间(秒)             |
| paper-airplane.tcp         | bool   | true                                     | 启用纸飞机TCP加速                  |
| http-proxy.bind            | string | 192.168.0.24                             | HTTP代理绑定地址             |
| http-proxy.port            | int    | 8080                                     | HTTP代理端口                 |
| socks-proxy.bind           | string | 192.168.0.24                             | SOCKS代理绑定地址            |
| socks-proxy.port           | int    | 1080                                     | SOCKS代理端口                |
| socks-proxy.username       | string | test                                     | SOCKS认证用户名              |
| socks-proxy.password       | string | 123456                                   | SOCKS认证密码                |

#### 📍 端口映射 (mappings)
| 参数名       | 类型   | 值示例         | 说明                          |
|--------------|--------|----------------|-----------------------------|
| local-ip     | string | 192.168.0.24   | 本地IP地址                   |
| local-port   | int    | 80             | 本地端口                     |
| protocol     | string | tcp            | 协议类型(tcp/udp)           |
| remote-ip    | string | ::             | 远程IP(::表示任意)          |
| remote-port  | int    | 10001          | 远程端口                     |

#### 🛣️ 路由规则 (routes)
| 参数名   | 类型   | 值示例                         | 说明                          |
|----------|--------|--------------------------------|-----------------------------|
| name     | string | CMNET                         | 路由规则名称                 |
| nic      | string | eth1                          | 网络接口名称                 |
| ngw      | string | 192.168.1.1                   | 网关地址                     |
| path     | string | ./cmcc_cidr.txt               | 本地CIDR文件路径             |
| vbgp     | string | https://ispip.clang.cn/cmcc_cidr.txt     | 在线CIDR数据源URL |
...
## 附录1
### UDP 宽频聚合器设置
当 `udp.static.aggligator` 设置大于0时，必须配置 `servers` 数组：

```json
"udp": {
    "static": {
        //...其他配置
        "aggligator": 2,          // 启用聚合器（值>0表示启用）
        "servers": [              // 必须配置转发服务器
            "192.168.1.100:6000", 
            "10.0.0.2:6000"
        ]
