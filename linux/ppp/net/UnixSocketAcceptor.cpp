...
namespace ppp
{
    namespace net
    {
        UnixSocketAcceptor::UnixSocketAcceptor() noexcept
            ...
            , context_(NULL)
        {
            context_ = ppp::threading::Executors::GetDefault().get();
        }
        ...
        {
            boost::asio::io_context* context = context_;
            if (NULL == context)
            {
                return false;
            }
        ...
        bool UnixSocketAcceptor::Open(const char* localIP, int localPort, int backlog) noexcept
        {
            ...
            boost::asio::ip::address address = boost::asio::ip::make_address(localIP, ec);
            if (ec)
            {
                return false;
            }

            server_ = make_shared_object<boost::asio::ip::tcp::acceptor>(*context_);
            if (NULL == server_)
            {
                return false;
            }
        ...
        void UnixSocketAcceptor::Dispose() noexcept
        {
            ...
            if (NULL != context)
            {
                ...
                context->post([self, this]() noexcept
                    {
                        Finalize();
                    });
            }
        ...
        bool UnixSocketAcceptor::Next() noexcept
        {
            ...
            server->async_accept(*socket, [self, this, server, socket](boost::system::error_code ec) noexcept
                {
