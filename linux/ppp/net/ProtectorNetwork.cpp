...
namespace ppp
{
    namespace net
    {
        ...
        bool ProtectorNetwork::MainJNI(JNIEnv* env, void* state, MainJNIStart start) noexcept
        {
            if (NULL == env)
            {
                return false;
            ...
            else
            {
                ...
                {
                    context->post(
                        [context, env, state, start]() noexcept
                        {
                            start(context, env, state);
                        });
                }
        ...
        bool ProtectorNetwork::ProtectJNI(const std::shared_ptr<boost::asio::io_context>& context, int sockfd, YieldContext& y) noexcept
        {
            ...
            context->post(
                [self, this, &ok, &y, sockfd]() noexcept
                {
                    ...
                    std::shared_ptr<boost::asio::io_context> jni = jni_;
                    if (NULL != jni)
                    {
                        JNIEnv* env = env_;
                        if (NULL != env)
        ...
        void ProtectorNetwork::ExitJNI() noexcept
        {
            std::shared_ptr<boost::asio::io_context> context = jni_;
            if (NULL != context)
            {
                ppp::threading::Executors::Exit(context);
            }
        }
#endif
