    ...
    namespace tap {
        ...
        bool TapLinux::AddRoute(const ppp::string& ifrName, UInt32 address, int prefix, UInt32 gw) noexcept {
            if (ifc_ctl_sock.compatible_route) {
                ...
                char cmd[1000];
                if (prefix > 31) {
                    len = snprintf(cmd, sizeof(cmd), "route add -host %s gw %s", address_string.data(), gw_string.data());
                }
                else {
                    ppp::string netmask_string = IPEndPoint::ToAddressString(IPEndPoint::PrefixToNetmask(prefix));
                    len = snprintf(cmd, sizeof(cmd), "route add -net %s netmask %s gw %s",
                        address_string.data(),
                        netmask_string.data(),
                        gw_string.data());
                }

                return len > 0 ? system(cmd) == 0 : false;
            }
            else {
        ...
        bool TapLinux::DeleteRoute(const ppp::string& ifrName, UInt32 address, int prefix, UInt32 gw) noexcept {
            if (ifc_ctl_sock.compatible_route) {
                ...
                char cmd[1000];
                if (prefix > 31) {
                    len = snprintf(cmd, sizeof(cmd), "route add -host %s gw %s",
                        address_string.data(),
                        gw_string.data());
                }
                else {
                    len = snprintf(cmd, sizeof(cmd), "route add -host %s/%d gw %s",
                        address_string.data(),
                        prefix,
                        gw_string.data());
                }
