...
void PppApplication::PullIPList(const ppp::string& command) noexcept
{
    ...
    ppp::string path;
    ppp::string nation;
    if (ppp::string command_string = ppp::LTrim(ppp::RTrim(command)); command_string.size() > 0) 
    {
...
int PppApplication::Main(int argc, const char* argv[]) noexcept
{
    ...
    ppp::string rerun_name = (client_or_server ? "client://" : "server://") + configuration_path_;
