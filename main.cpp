        ...
        chnroutes2_getiplist_async(
            [path, nation](const ppp::string& iplist) noexcept 
            {
                ...
                while (chnroutes2_getiplist(ips, nation, iplist))
                {
                    ...
                    if (chnroutes2_getiplist(olds, ppp::string(), ppp::LTrim(ppp::RTrim(File::ReadAllText(path.data())))) > 0 && !chnroutes2_equals(ips, olds))
                    {
...
void PppApplication::GetDnsAddresses(ppp::vector<boost::asio::ip::address>& addresses, int argc, const char* argv[]) noexcept
{
    ...
    Ipep::ToDnsAddresses(dns, addresses);
}
...
bool PppApplication::OnTick(uint64_t now) noexcept
{
    ...
    if (uint64_t delta_time = (now - PPP_APPLICATION_DEFAULT_APP_DOMAIN_STATIC.auto_pull_iplist_last) / 1000; delta_time >= PPP_AUTO_PULL_IPLIST_INTERVAL)
    {
        ...
        if (RouteIPListTablePtr vbgp = client->GetVbgp(); PPP_APPLICATION_DEFAULT_APP_DOMAIN_STATIC.vbgp && NULL != vbgp)
        {
