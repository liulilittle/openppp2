...
| Command | Function | Format | Default |
|---------|----------|--------|---------|
| `--lwip` | Protocol stack selection | `--lwip=[yes｜no]` | Windows: `yes`<br>Others: `no` |
...
### <img src="https://icons.veryicon.com/png/o/business/icons-related-to-operation-and-maintenance/cloud-gateway.png" width="20" height="20"> Virtual Gateway Address Default Values
| Platform | Default Value |
|----------|--------------|
| Windows | `10.0.0.0` |
| Linux | `10.0.0.1` |
...
### <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRW-HldTH5IkPK9M8NePfmj0f6EeeCDEI9SVw&s" width="20" height="20"> Symbol Explanation
| Symbol | Explanation |
|:--:|--------------|
| `[ ]` | Optional parameter |
| `< >` | Required parameter |
...
### SIMD + AES_NI Optimization Version
...
|----------------|-------|---------|--------------|
| concurrent     | int   | 1       | [Concurrency thread count](#concurrent-number-guide) |
| cdn            | array | [80,443] | Ports used by CDN |

### 🔑 Encryption Settings (key)

| Parameter Name  | Type    | Value Example       | Description                 |
|-----------------|---------|---------------------|----------------------------|
| kf             | int     | 154543927           | Key generation factor      |
| kx             | int     | 128                 | Interleaving factor        |
| kl             | int     | 10                  | NOP min bits               |
| kh             | int     | 12                  | NOP max bits               |
| sb             | int     | 1000                | Dynamic buffer size (bytes)|
| protocol       | string  | aes-128-cfb         | Protocol layer encryption algorithm |
| protocol-key   | string  | N6HMzdUs7IUnYHwq    | Protocol layer encryption key |
| transport      | string  | aes-256-cfb         | Transport layer encryption algorithm |
| transport-key  | string  | HWFweXu2g5RVMEpy    | Transport layer encryption key |
| masked         | bool    | false               | Enable traffic masking     |
| plaintext      | bool    | false               | Allow plaintext transmission |
| delta-encode   | bool    | false               | Enable delta encoding      |
| shuffle-data   | bool    | false               | Enable data randomization  |

### 📡 Network Interface (ip)

| Parameter Name | Type | Example Value | Description |
|----------------|-------|----------------|--------------|
| public         | string | 192.168.0.24  | Public IP address |
| interface      | string | 192.168.0.24  | Local listening interface IP |

### 💾 Virtual Memory (vmem)

| Parameter Name | Type | Example Value | Description |
|----------------|-------|----------------|--------------|
| size           | int   | 4096           | Memory pool size (MB) |
| path           | string | "./{}"        | Memory file storage path |

### 🔌 TCP Settings

| Parameter Name | Type | Example Value | Description |
|----------------|-------|----------------|--------------|
| inactive.timeout | int | 300 | Connection idle timeout (seconds) |
| connect.timeout  | int | 5 | Connection establishment timeout (seconds) |
| listen.port      | int | 20000 | Listening port |
| cwnd             | int | 0 | Congestion window size (auto-adjusted) |
| rwnd             | int | 0 | Receive window size (auto-adjusted) |
| turbo            | bool | true | Enable TCP acceleration |
| backlog          | int | 511 | Max pending connections |
| fast-open        | bool | true | Enable TCP Fast Open |

### 📶 UDP Settings

| Parameter Name | Type | Example Value | Description |
|----------------|-------|----------------|--------------|
| cwnd           | int   | 0 | Congestion window size |
| rwnd           | int   | 0 | Receive window size |
| inactive.timeout | int | 72 | Idle timeout (seconds) |
| dns.timeout    | int   | 4 | DNS query timeout (seconds) |
| dns.ttl        | int   | 60 | DNS cache TTL (seconds) |
| dns.cache      | bool  | true | Enable DNS cache |
| dns.redirect   | string | "0.0.0.0" | DNS redirect address |
| listen.port    | int   | 20000 | Listening port |
| static.keep-alived | array | [1,5] | Keep-alive interval [min,max] (seconds) |
| static.dns     | bool  | true | Enable static DNS service |
| static.quic    | bool  | true | Enable QUIC support |
| static.icmp    | bool  | true | Enable ICMP support |
| static.aggligator | int | 4 | Bandwidth aggregator links |
| static.servers | array | ["1.0.0.1:20000",...] | Static server address list |

### 🔄 Multiplexing (mux)

| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| connect.timeout | int | 20 | Connection establishment timeout (seconds) |
| inactive.timeout | int | 60 | Stream idle timeout (seconds) |
| congestions | int | 134217728 | Max congestion control window (bytes) |
| keep-alived | array | [1,20] | Keep-alive interval [min,max] (seconds) |

### 🌐 WebSocket Settings

| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| host | string | starrylink.net | Server domain name |
| path | string | /tun | WebSocket path |
| listen.ws | int | 20080 | HTTP listen port |
| listen.wss | int | 20443 | HTTPS listen port |
| ssl.certificate-file | string | starrylink.net.pem | SSL certificate file |
| ssl.certificate-chain-file | string | starrylink.net.pem | SSL certificate chain file |
| ssl.certificate-key-file | string | starrylink.net.key | SSL private key file |
| ssl.certificate-key-password | string | test | SSL private key password |
| ssl.ciphersuites | string | TLS_AES_256_GCM_SHA384:... | Cipher suites list |
| verify-peer | bool | true | Verify client certificate |
| http.error | string | Status Code: 404; Not Found | HTTP error response content |
| http.request | object | {Cache-Control: "no-cache", ...} | Custom HTTP request headers |
| http.response | object | {Server: "Kestrel"} | Custom HTTP response headers |

### 🖥️ Server Configuration (server)

| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| log | string | ./ppp.log | Log file path |
| node | int | 1 | Server node ID |
| subnet | bool | true | Enable subnet allocation |
| mapping | bool | true | Enable port mapping |
| backend | string | ws://192.168.0.24/ppp/webhook | Management backend URL |
| backend-key | string | HaEkTB55VcHovKtUPHmU9zn0NjFmC6tff | Management backend authentication key |

### 💻 Client Configuration (client)

| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| guid | string | {F4569208-BB45-4DEB-B115-0FEA1D91B85B} | Unique client identifier |
| server | string | ppp://192.168.0.24:20000/ | Server connection address |
| server-proxy | string | http://user:pass@192.168.0.18:8080/ | Proxy address for connection |
| bandwidth | int | 10000 | Bandwidth limit (Kbp/s) |
| reconnections.timeout | int | 5 | Reconnection wait time (seconds) |
| paper-airplane.tcp | bool | true | Enable PaperAirplane TCP acceleration |
| http-proxy.bind | string | 192.168.0.24 | HTTP proxy bind address |
| http-proxy.port | int | 8080 | HTTP proxy port |
| socks-proxy.bind | string | 192.168.0.24 | SOCKS proxy bind address |
| socks-proxy.port | int | 1080 | SOCKS proxy port |
| socks-proxy.username | string | test | SOCKS authentication username |
| socks-proxy.password | string | 123456 | SOCKS authentication password |

#### 📍 Port Mapping (mappings)
| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| local-ip | string | 192.168.0.24 | Local IP address |
| local-port | int | 80 | Local port |
| protocol | string | tcp | Protocol type (tcp/udp) |
| remote-ip | string | :: | Remote IP (:: for any) |
| remote-port | int | 10001 | Remote port |

#### 🛣️ Routing Rules (routes)
| Parameter Name | Type | Example | Description |
|----------------|-------|---------|--------------|
| name | string | CMNET | Route rule name |
| nic | string | eth1 | Network interface name |
| ngw | string | 192.168.1.1 | Gateway address |
| path | string | ./cmcc_cidr.txt | Local CIDR file path |
| vbgp | string | https://ispip.clang.cn/cmcc_cidr.txt | Online CIDR data source URL |
...
When `udp.static.aggligator` is set greater than 0, configure `servers` array:

```json
"udp": {
    "static": {
        //... other settings
        "aggligator": 2,          // Enable aggregator (value >0)
        "servers": [              // Must configure forwarding servers
            "192.168.1.100:6000", 
            "10.0.0.2:6000"
        ]
    }
