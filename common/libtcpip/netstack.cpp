...
namespace lwip {
    ...
    void netstack_cctor() noexcept {
        netstack::Executor = ppp::make_shared_object<boost::asio::io_context>();
    }
        ...
        if (socket_) {
            ...
            while (!sents.empty()) { // tcp_sndbuf
                ...
                do {
                    ...
                    err_ = tcp_write(pcb,
                        context->buf.p.get(),
                        context->buf.sz, TCP_WRITE_FLAG_COPY);
                    if (err_ == ERR_OK) {
                        tcp_output(pcb);
                        if (context->cb) {
                            context->cb(pcb);
    ...
    static err_t netstack_tcp_doaccept(void* arg, struct tcp_pcb* pcb, err_t err) noexcept {
        ...
        socket_->pnat = 0;
        socket_->open = false;
        socket_->socket = std::make_shared<boost::asio::ip::tcp::socket>(context);
    ...
    bool netstack::open() noexcept {
        ...
        if (timeout_) {
            ...
            timeout_ = std::make_shared<boost::asio::deadline_timer>(context);
        }
