...
void                                                                        libopenppp2_application::DllMain() noexcept {
    ...
    if (max_concurrent > 1) {
        ...
        Executors::SetMaxThreads(allocator, max_concurrent - 1);
    }
}
...
static int                                                                          libopenppp_try_open_ethernet_switcher_new(
    ...
    if (std::shared_ptr<ppp::string> bypass_ip_list = std::move(app->bypass_ip_list_); NULL != bypass_ip_list) {
        app->bypass_ip_list_.reset();
        client->SetBypassIpList(std::move(*bypass_ip_list));
    }

    if (std::shared_ptr<ppp::string> dns_rules_list = std::move(app->dns_rules_list_); NULL != dns_rules_list) {
