...
namespace ppp
{
    namespace tap
    {
        typedef enum
        {
            ...
            FILE_ANY_ACCESS = 0
        } TAP_WIN_CONTROL_CODES;

        static constexpr uint32_t CTL_CODE(uint32_t DeviceType, uint32_t Function, uint32_t Method, uint32_t Access) noexcept
        {
            return ((DeviceType) << 16) | ((Access) << 14) | ((Function) << 2) | (Method);
        }

        static constexpr uint32_t TAP_WIN_CONTROL_CODE(uint32_t request, uint32_t method) noexcept
        {
...
boost::uuids::uuid reft_id = StringToGuid(componentId);
        ...
        bool TapWindows::ConfigureDriver_SetNetifTunMode(const void* handle, uint32_t ip, uint32_t gw, uint32_t mask) noexcept
        {
            ...
            return ppp::win32::Win32Native::DeviceIoControl(handle, TAP_WIN_IOCTL_CONFIG_TUN, ep, sizeof(ep));
        }
